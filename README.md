...
- [`workflows/RH_Krea2_Offline_Qwen3VL_Klein_MinimaxH3_T2VA_Audio_v9.0.json`](workflows/RH_Krea2_Offline_Qwen3VL_Klein_MinimaxH3_T2VA_Audio_v9.0.json) keeps the offline Qwen3VL/Krea2/Klein storyboard pipeline and uses DaSiWa MiniMax H3 as its only video branch. The nested T2VA + audio output is muted by default and is controlled by the visible `MiniMax H3 视频开关` rgthree toggle. Its tested active defaults are 18 seconds, 0.65 MP, square output and 24 FPS. `RH_REF2V_STORYBOARD_PROMPT` uses text-only references, cinematic diegetic sound design and no non-diegetic music.
