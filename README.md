...
- [`workflows/RH_Krea2_Manual_Prompt_Override.json`](workflows/RH_Krea2_Manual_Prompt_Override.json) is based on the supplied Krea2/Qwen/MiniMax workflow and adds an automatic/manual final-prompt switch. Repeated character/style/aspect locks are shown once as a Global Prompt, while the clean Scene Prompt list remains easy to copy and edit; the two layers are recombined only on the Krea generation path.
