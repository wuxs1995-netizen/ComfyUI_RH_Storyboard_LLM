...
### RH Storyboard - Auto / Manual Prompt Source
...
The manual field accepts a JSON string array, one complete prompt per line, or multiline prompts separated by a line containing `---`. The bundled `workflows/RH_Krea2_Manual_Prompt_Override.json` example places this switch immediately after the locked parser, so the same selected list feeds Krea, the final prompt preview and MiniMax prompt compilation.
...
### RH Storyboard - MiniMax H3 Image Guide

The MiniMax H3 adapter nodes pass generated storyboard frames directly to DaSiWa without writing temporary image files:
...
- [`workflows/RH_Krea2_Manual_Prompt_Override.json`](workflows/RH_Krea2_Manual_Prompt_Override.json) is based on the supplied Krea2/Qwen/MiniMax workflow and adds an automatic/manual final-prompt switch. In manual mode, paste edited final prompts and queue the workflow without rerunning Qwen.
- [`workflows/RH_Krea2_Offline_Qwen3VL_Klein_MinimaxH3_StoryboardModes_v9.1.json`](workflows/RH_Krea2_Offline_Qwen3VL_Klein_MinimaxH3_StoryboardModes_v9.1.json) is the current DaSiWa-only video workflow. It adds direct storyboard-image transport for T2VA, I2VA, L2VA, FL2VA and REF2VA, with FL2VA selected by default. One settings node drives both the RH image guide and the nested MiniMax Director, so mode and duration cannot drift apart. The visible mapping preview reports which scene images were selected.
- [`workflows/RH_Krea2_Offline_Qwen3VL_Klein_MinimaxH3_T2VA_Audio_v9.0.json`](workflows/RH_Krea2_Offline_Qwen3VL_Klein_MinimaxH3_T2VA_Audio_v9.0.json) is retained as the earlier text-only MiniMax T2VA workflow.
...
The v9.0/v9.1 MiniMax workflows require rgthree, ComfyUI-DaSiWa-Nodes and the MiniMax H3 model files referenced by the workflow: `minimax_h3_fl2va_pruned_int8_convrot.safetensors`, `minimax_h3_ref2va_pruned_int8_convrot.safetensors`, `qwen3vl_32b_minimax_h3_nvfp4_awq.safetensors`, `minimax_h3_video_vae_fp16.safetensors` and `minimax_h3_audio_vae_fp32.safetensors`. Leave the MiniMax toggle muted when only storyboard images are needed.
